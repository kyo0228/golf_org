...
class MY_Controller extends GC_Controller
{
	...
	public function __construct()
	{
		...
		if (!$group_code) {
			//パラメータなしはadsence用のデモユーザーとして扱う
			$this->page->set_item("demo_user", true);

			//ひとまずハードコーディング
			$group_data = array(
				'group_id' => 0,
				'group_code' => 'golfcompescore',
				'group_name' => 'ゴルフコンペスコア',
				'admin_pw' => 'admin', //LwqqOhSGgby7D1ky#9Ar
				'member_pw' => 'k-souma',
				'month_start' => 1,
				'month_end' => 12,
				'invalid_flg' => 0,
			...
			$this->page->set_item("demo_user", false);
		...
		if (is_uploaded_file($file["tmp_name"])) {
			...
			switch (strtolower($ext)) {
				...
				case "gif":
					...
					break;
				case "bmp":
					$img = @$this->imagecreatefrombmp($file["tmp_name"]);
					break; //bmpは独自関数
			}
			...
			switch (strtolower($ext)) {
				...
				case "jpeg":
					...
					break;
				case "png":
					$ret = imagepng($canvas, $dir . $saveName);
					break;
				case "bmp":
					//case "png":
				case "gif":
