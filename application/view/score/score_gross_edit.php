  ...
  <div class="col-lg-12 col-md-12">
    <div class="card">
      <div class="card-header card-header-warning">
        <h4 class="card-title"><?=$this->value("compe_data","compe_name")?>　参加者一覧　グロス一括登録</h4>
      </div>
      <!--<form>-->
        ...
        <div class="card-body">
          <div class="score_other">
            <div class="form-group x2">
              <label class="bmd-label-floating"><?=$row["member_name"]?>(hc:<?=$row["handicap"]?>)</label>
              <input type="number" class="form-control" id="<?='gross_'.$row["member_id"]?>" value="<?=$row["gross"]?>" onchange="score_change(<?=$row["member_id"]?>)">
              <input type="hidden" id="<?='handicap_'.$row["member_id"]?>" value="<?=$row["handicap"]?>">
            </div>
          </div>        
        </div>                  
